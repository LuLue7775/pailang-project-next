    ...
    <StyledAnimTitle
      ...
      {[...title].map((letter, i) => (
        <StyledAnimTitle
          ...
          isTitleExceed={textLengthRef.current} 
          windowWidth={windowWidth}
          style={{ fontFamily: language === 'zh' && 'Noto Serif TC', fontSize: language === 'zh' && '1.5rem' }  }
        >
          {letter}
        </StyledAnimTitle>
      ))}
...
export default function ArticlesHeader({ data, slideTo, spring }) {
  ...
  return (
    <StyledHeader>
        ...
        <StyledTitles>
            <div>
              <AnimatedTitles title={data?.title} language={'en'} textLengthRef={titleRef} windowWidth={windowWidth}/>
            </div>
            <div>
              <AnimatedTitles title={data?.title_zh} language={'zh'} textLengthRef={titleZhRef} windowWidth={windowWidth}/>
            </div>
...
const StyledAnimTitle = styled(motion.span)`
  ...
  // font-family: ${({ language }) => language === 'en' ? 'var( --title-font-en, serif);' : '"Noto Serif TC", serif);'}
